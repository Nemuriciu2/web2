...
const FILMS = [
  ...
  router.get('/', (req, res, next) => {
    ...
    const filter = req?.query['minimum-duration']?.includes('value') ? req.query['minimum-duration'] : undefined;
    let orderedFilm;

    if(filter){
      orderedFilm = [...FILMS].sort((a,b) => a.duration - b.duration);
    }
    if(filter === '-value'){
      orderedFilm = orderedFilm.reverse();
    }
